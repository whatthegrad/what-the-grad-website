...
export default function FoundersPage() {
  return (
    <div style={{ minHeight: '100vh', fontFamily: PD }}>
      ...
      <div style={{
        background: '#D6E8F5', minHeight: '52vh',
        ...
        <div style={{ position:'relative', zIndex:2, textAlign:'center' }}>
          <p style={{ fontFamily:PD, fontSize:'13px', letterSpacing:'0.2em', textTransform:'uppercase', color:'rgba(44,24,16,0.4)', marginBottom:'20px', animation:'fadeUp 0.8s ease both' }}>
            The people behind What The Grad
          </p>
          <h1 style={{ fontFamily:PD, fontSize:'clamp(40px, 6vw, 80px)', fontWeight:'700', color:'#2C1810', letterSpacing:'-0.02em', lineHeight:'1.1', animation:'glow 2.8s ease-in-out infinite, fadeUp 1s ease 0.2s both' }}>
            Meet our Founders
          </h1>
          <p style={{ fontFamily:PD, fontSize:'clamp(15px, 1.8vw, 20px)', fontStyle:'italic', color:'#5B8FA8', margin:'20px auto 0', maxWidth:'560px', animation:'fadeUp 1s ease 0.4s both' }}>
            Two friends. One leap. A whole lot of clarity to give.
          </p>
        </div>
        <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'80px', background:'linear-gradient(to bottom, transparent, #ffffff)', pointerEvents:'none' }} />
      </div>
      ...
      <style>{`
        ...
        @keyframes throb { 0%,100% { transform:scale(0.7) rotate(0deg); opacity:0.4; } 50% { transform:scale(1.3) rotate(20deg); opacity:1; } }
      `}</style>
    </div>
  );
}
