...
export default function ServicesSection() {
  ...
  const handleAddToCart = (e: React.MouseEvent, pkg: typeof PACKAGES[0]) => {
    e.stopPropagation();
    addItem({ id: pkg.id, name: pkg.name, price: pkg.price });
    setAdded(pkg.id);
            ...
            return (
              ...
              >
                ...
                <div style={{ padding: '28px 28px 32px' }}>
                  <h3 style={{ fontFamily: PD, fontSize: '22px', fontWeight: '700', color: '#2C1810', marginBottom: '6px' }}>{pkg.name}</h3>
                  <p style={{ fontFamily: PD, fontSize: '13px', color: '#9B8B7A', fontStyle: 'italic', marginBottom: '16px' }}>{pkg.tagline}</p>
                  <p style={{ fontFamily: PD, fontSize: '14px', color: '#5C4A3A', lineHeight: '1.6', marginBottom: '24px' }}>{pkg.shortDesc}</p>

                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                    <span style={{ fontFamily: PD, fontSize: '26px', fontWeight: '700', color: '#2C1810' }}>{pkg.price}</span>
