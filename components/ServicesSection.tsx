...
export default function ServicesSection() {
  ...
  if (isMobile) {
    return (
      <section id="services" ref={sectionRef} style={{ background: 'linear-gradient(180deg, #D6E8F5 0%, #FFF8EC 15%, #FFF8EC 100%)', padding: '56px 5vw 0' }}>
          ...
          {PACKAGES.map((pkg) => {
            ...
            return (
              <div key={pkg.id} style={{
                background: '#FFFEF9', borderRadius: '20px', overflow: 'hidden',
                boxShadow: '0 4px 20px rgba(44,24,16,0.08)',
                border: pkg.badge ? '2px solid #F5A623' : '1.5px solid rgba(44,24,16,0.07)',
                position: 'relative',
              }}>
                {pkg.badge && (
                  <div style={{ position:'absolute', top:'12px', left:'12px', background:'#F5A623', color:'#2C1810', fontFamily:PD, fontSize:'10px', fontWeight:'700', letterSpacing:'0.08em', textTransform:'uppercase', padding:'4px 10px', borderRadius:'100px', zIndex:2 }}>★ {pkg.badge}</div>
                )}
        ...
        {/* Add-ons mobile */}
        <div style={{ position:'relative', marginTop:'0', padding:'56px 0 72px' }}>
          <div style={{ position:'absolute', inset:0, backgroundImage:'url(/images/linen-bg.jpg)', backgroundSize:'cover', backgroundPosition:'center', opacity:0.85 }}/>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:'80px', background:'linear-gradient(to bottom, #FFF8EC 0%, transparent 100%)', zIndex:1, pointerEvents:'none' }}/>
          <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'80px', background:'linear-gradient(to top, #FFF8EC 0%, transparent 100%)', zIndex:1, pointerEvents:'none' }}/>
          <div style={{ position:'relative', zIndex:2, padding:'0 5vw' }}>
            ...
            <h3 style={{ fontFamily:PD, fontSize:'clamp(20px, 6vw, 28px)', fontWeight:'700', color:'#2C1810', letterSpacing:'-0.02em', marginBottom:'24px', textAlign:'center' }}>Bolt-on services</h3>
            <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
              {ADDONS.map((addon) => {
                const justAdded = added === addon.id;
                return (
                  <div key={addon.id} style={{ background:'rgba(255,255,255,0.82)', backdropFilter:'blur(8px)', border:'1px solid rgba(255,255,255,0.9)', borderRadius:'14px', padding:'14px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'12px' }}>
                    <div style={{ flex:1 }}>
                    ...
                    <button onClick={(e) => handleAddToCart(e, addon)} style={{ padding:'8px 16px', background: justAdded ? '#22C55E' : '#2C1810', color: justAdded ? 'white' : '#FFF8EC', border:'none', borderRadius:'100px', fontFamily:PD, fontSize:'12px', fontWeight:'700', cursor:'pointer', whiteSpace:'nowrap', transition:'background 0.3s' }}>
                      {justAdded ? 'Added' : '+ Add'}
                    </button>
                  </div>
                );
  ...
  // ── DESKTOP ────────────────────────────────────────────────────────────────
  return (
    <section id="services" ref={sectionRef} style={{ height:'500vh', position:'relative', background:'linear-gradient(180deg, #D6E8F5 0%, #FFF8EC 20%, #FFF8EC 100%)' }}>
      ...
      <div style={{ position:'sticky', top:0, height:'100vh', overflow:'hidden', display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center' }}>
        ...
        <div ref={trackRef} style={{ display:'flex', gap:'28px', alignItems:'stretch', transform:`translateX(${trackTranslate}%)`, transition:'transform 0.05s linear', willChange:'transform', padding:'0 8vw', marginTop:'60px' }}>
          {PACKAGES.map((pkg, i) => {
      ...
      <div style={{ position:'absolute', bottom:0, left:0, right:0 }}>
        <div style={{ position:'relative', padding:'100px 5vw 100px' }}>
          <div style={{ position:'absolute', inset:0, backgroundImage:'url(/images/linen-bg.jpg)', backgroundSize:'cover', backgroundPosition:'center', opacity:0.88 }}/>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:'140px', background:'linear-gradient(to bottom, #FFF8EC 0%, transparent 100%)', zIndex:1, pointerEvents:'none' }}/>
          <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'140px', background:'linear-gradient(to top, #FFF8EC 0%, transparent 100%)', zIndex:1, pointerEvents:'none' }}/>

          <div style={{ maxWidth:'1100px', margin:'0 auto', position:'relative', zIndex:2 }}>
            <div style={{ textAlign:'center', marginBottom:'52px' }}>
              <p style={{ fontFamily:PD, fontSize:'12px', letterSpacing:'0.18em', textTransform:'uppercase', color:'#9B8B7A', marginBottom:'8px' }}>Bolt-on services</p>
              <h3 style={{ fontFamily:PD, fontSize:'clamp(24px, 3vw, 36px)', fontWeight:'700', color:'#2C1810', letterSpacing:'-0.02em' }}>Add-ons to any package</h3>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:'16px' }}>
              {ADDONS.map((addon) => {
                ...
                return (
                  <div
                    key={addon.id}
                    style={{ background:'rgba(255,255,255,0.78)', backdropFilter:'blur(10px)', border:'1px solid rgba(255,255,255,0.95)', borderRadius:'16px', padding:'20px', display:'flex', flexDirection:'column', gap:'8px', transition:'box-shadow 0.2s, transform 0.2s' }}
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{ padding:'10px', background: justAdded ? '#22C55E' : 'transparent', color: justAdded ? 'white' : '#2C1810', border:`1.5px solid ${justAdded ? '#22C55E' : 'rgba(44,24,16,0.2)'}`, borderRadius:'100px', fontFamily:PD, fontSize:'12px', fontWeight:'700', cursor:'pointer', transition:'all 0.25s' }}
